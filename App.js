...
const BOARD_SIZES={small:{cols:4,rows:4,name:'Iniciante (4x4)'},classic:{cols:6,rows:4,name:'Clássico (6x4)'},medium:{cols:8,rows:4,name:'Médio (8x4)'},large:{cols:10,rows:4,name:'Grande (10x4)'}};
const INITIAL_SEEDS_OPTIONS=[{value:2,name:'2 Dados (Rápido)'},{value:3,name:'3 Dados (Normal)'},{value:4,name:'4 Dados (Lento)'},{value:5,name:'5 Dados (Muito Lento)'}];
const DEFAULT_COLS=6,DEFAULT_ROWS=4,DEFAULT_TOTAL=DEFAULT_ROWS*DEFAULT_COLS,INITIAL_SEEDS=2;
const IDX={P2_DEF:0,P2_ATK:1,P1_ATK:2,P1_DEF:3};
const THEMES={savanna:{name:'Savana',primary:'#D97706',secondary:'#92400E',accent:'#FCD34D',bg:['#1C0F05','#2D1810'],boardBg:'rgba(139,69,19,0.3)',cellBg:'rgba(101,67,33,0.6)',cellBorder:'rgba(217,119,6,0.5)',seed:'#F59E0B',text:'#FEF3C7',emoji:'🌾',p1Color:'#3B82F6',p1Shadow:'#60A5FA',p2Color:'#EF4444',p2Shadow:'#FCA5A5'},sunset:{name:'Por do Sol',primary:'#DC2626',secondary:'#7C2D12',accent:'#FBBF24',bg:['#450A0A','#7C2D12'],boardBg:'rgba(153,27,27,0.3)',cellBg:'rgba(127,29,29,0.6)',cellBorder:'rgba(220,38,38,0.5)',seed:'#EF4444',text:'#FEE2E2',emoji:'🌅',p1Color:'#3B82F6',p1Shadow:'#60A5FA',p2Color:'#EF4444',p2Shadow:'#FCA5A5'},forest:{name:'Floresta',primary:'#059669',secondary:'#064E3B',accent:'#34D399',bg:['#022C22','#064E3B'],boardBg:'rgba(6,78,59,0.3)',cellBg:'rgba(6,95,70,0.6)',cellBorder:'rgba(5,150,105,0.5)',seed:'#10B981',text:'#D1FAE5',emoji:'🌳',p1Color:'#3B82F6',p1Shadow:'#60A5FA',p2Color:'#EF4444',p2Shadow:'#FCA5A5'},desert:{name:'Deserto',primary:'#CA8A04',secondary:'#713F12',accent:'#FDE047',bg:['#422006','#713F12'],boardBg:'rgba(113,63,18,0.3)',cellBg:'rgba(161,98,7,0.6)',cellBorder:'rgba(202,138,4,0.5)',seed:'#FACC15',text:'#FEF9C3',emoji:'🏜️',p1Color:'#3B82F6',p1Shadow:'#60A5FA',p2Color:'#EF4444',p2Shadow:'#FCA5A5'},ocean:{name:'Oceano',primary:'#0284C7',secondary:'#0C4A6E',accent:'#38BDF8',bg:['#082F49','#0C4A6E'],boardBg:'rgba(12,74,110,0.3)',cellBg:'rgba(7,89,133,0.6)',cellBorder:'rgba(2,132,199,0.5)',seed:'#0EA5E9',text:'#E0F2FE',emoji:'🌊',p1Color:'#3B82F6',p1Shadow:'#60A5FA',p2Color:'#EF4444',p2Shadow:'#FCA5A5'}};

function idxToRowCol(idx,COLS){return{row:Math.floor(idx/COLS),col:idx%COLS}}
function rowColToIdx(row,col,COLS){return row*COLS+col}
function nextIndex(idx,player,COLS){const{row,col}=idxToRowCol(idx,COLS);if(player===1){if(row===IDX.P1_DEF)return col<COLS-1?rowColToIdx(row,col+1,COLS):rowColToIdx(IDX.P1_ATK,COLS-1,COLS);if(row===IDX.P1_ATK)return col>0?rowColToIdx(row,col-1,COLS):rowColToIdx(IDX.P1_DEF,0,COLS)}else{if(row===IDX.P2_ATK)return col<COLS-1?rowColToIdx(row,col+1,COLS):rowColToIdx(IDX.P2_DEF,COLS-1,COLS);if(row===IDX.P2_DEF)return col>0?rowColToIdx(row,col-1,COLS):rowColToIdx(IDX.P2_ATK,0,COLS)}return idx}
function isPlayerSide(idx,player,COLS){const{row}=idxToRowCol(idx,COLS);return player===1?(row===IDX.P1_ATK||row===IDX.P1_DEF):(row===IDX.P2_ATK||row===IDX.P2_DEF)}
function isAttackRow(idx,player,COLS){const{row}=idxToRowCol(idx,COLS);return player===1?row===IDX.P1_ATK:row===IDX.P2_ATK}
function isDefenseRow(idx,player,COLS){const{row}=idxToRowCol(idx,COLS);return player===1?row===IDX.P1_DEF:row===IDX.P2_DEF}
function isCornerHole(idx,COLS){const{col}=idxToRowCol(idx,COLS);return col===0||col===COLS-1}
function isMiddleHole(idx,COLS){const{col}=idxToRowCol(idx,COLS);return col>=1&&col<=COLS-2}
function opponentAttackHouse(idx,player,COLS){const{col}=idxToRowCol(idx,COLS);return player===1?rowColToIdx(IDX.P2_ATK,col,COLS):rowColToIdx(IDX.P1_ATK,col,COLS)}
function opponentDefenseHouse(idx,player,COLS){const{col}=idxToRowCol(idx,COLS);return player===1?rowColToIdx(IDX.P2_DEF,col,COLS):rowColToIdx(IDX.P1_DEF,col,COLS)}
function createInitialBoard(TOTAL,seeds=INITIAL_SEEDS){return Array(TOTAL).fill(seeds)}
function getPlayerIndices(player,TOTAL,COLS){const res=[];for(let i=0;i<TOTAL;i++)if(isPlayerSide(i,player,COLS))res.push(i);return res}
function countPlayerPieces(board,player,COLS){return getPlayerIndices(player,board.length,COLS).reduce((sum,idx)=>sum+board[idx],0)}
function isPhaseTwo(board){return board.every(c=>c<=1)}
function getValidMoves(board,player,COLS){const moves=[],indices=getPlayerIndices(player,board.length,COLS),phase2=isPhaseTwo(board);for(const idx of indices){if(board[idx]===0)continue;if(phase2){if(board[idx]===1)moves.push(idx)}else moves.push(idx)}return moves}

function calculateMoveSteps(board,idx,player,COLS){
  const steps=[];
  let b=[...board],seeds=b[idx];
  b[idx]=0;
  let current=idx;
  const playerTotal=countPlayerPieces(board,player,COLS);
  ...
  while(continueLoop&&safety<100){
    ...
    if(isSuperSeed&&!superSeedProcessed){
      ...
      for(let i=0;i<3;i++){
        current=nextIndex(current,player,COLS);
        steps.push({type:'jump',idx:current,isSuperSeed})
      }
      if(isAttackRow(current,player,COLS)){
        const oppAtk=opponentAttackHouse(current,player,COLS),oppDef=opponentDefenseHouse(current,player,COLS);
    ...
    if(isAttackRow(current,player,COLS)){
      ...
      if(isMiddleHole(current,COLS)){
        continueLoop=false;
        break
      }
      if(isCornerHole(current,COLS)){
    ...
    }else if(isDefenseRow(current,player,COLS)){
      const pickedSeeds=b[current];
      b[current]=0;
      seeds=pickedSeeds;
      if(seeds>0){
...
function checkGameOver(board,currentPlayer,COLS){const p1Total=countPlayerPieces(board,1,COLS),p2Total=countPlayerPieces(board,2,COLS);if(p1Total===0)return{over:true,winner:2,reason:'Jogador 1 sem pecas'};if(p2Total===0)return{over:true,winner:1,reason:'Jogador 2 sem pecas'};const moves=getValidMoves(board,currentPlayer,COLS);if(moves.length===0){const opponent=currentPlayer===1?2:1;return{over:true,winner:opponent,reason:`Jogador ${currentPlayer} sem jogadas validas`}}return{over:false}}

function evaluateMove(board,idx,player,COLS){const steps=calculateMoveSteps(board,idx,player,COLS),finalBoard=steps[steps.length-1].board;let score=0;const opponent=player===1?2:1,oppBefore=countPlayerPieces(board,opponent,COLS),oppAfter=countPlayerPieces(finalBoard,opponent,COLS);score+=(oppBefore-oppAfter)*50;const ownAfter=countPlayerPieces(finalBoard,player,COLS);score+=ownAfter*2;return score}

function aiChooseMove(board,player,COLS){const moves=getValidMoves(board,player,COLS);if(moves.length===0)return null;let best=moves[0],bestScore=-Infinity;for(const idx of moves){const score=evaluateMove(board,idx,player,COLS);if(score>bestScore){bestScore=score;best=idx}}return best}

const{width:SCREEN_W,height:SCREEN_H}=Dimensions.get('window');
const getCellSize=(cols,rows)=>Math.min((SCREEN_W-24)/cols-6,Math.max(68,(SCREEN_H-240)/rows-8));

export default function App(){const[mode,setMode]=useState(null);const[board,setBoard]=useState(()=>createInitialBoard(DEFAULT_TOTAL));const[currentPlayer,setCurrentPlayer]=useState(1);const[gameOver,setGameOver]=useState(null);const[animationSteps,setAnimationSteps]=useState([]);const[currentStepIndex,setCurrentStepIndex]=useState(-1);const[isAnimating,setIsAnimating]=useState(false);const[currentAnimIdx,setCurrentAnimIdx]=useState(null);const[message,setMessage]=useState('');const[showHelp,setShowHelp]=useState(false);const[showSettings,setShowSettings]=useState(false);const[selectedTheme,setSelectedTheme]=useState('savanna');const[animationSpeed,setAnimationSpeed]=useState(500);const[soundEnabled,setSoundEnabled]=useState(true);const[isSuperSeedActive,setIsSuperSeedActive]=useState(false);const[boardSize,setBoardSize]=useState('classic');const[initialSeeds,setInitialSeeds]=useState(2);const[cols,setCols]=useState(DEFAULT_COLS);const[rows,setRows]=useState(DEFAULT_ROWS);const[total,setTotal]=useState(DEFAULT_TOTAL);const msgTimer=useRef(null);const pulseAnim=useRef(new Animated.Value(1)).current;const audioContextRef=useRef(null);const theme=THEMES[selectedTheme];
...
useEffect(()=>{const size=BOARD_SIZES[boardSize];if(size){setCols(size.cols);setRows(size.rows);setTotal(size.cols*size.rows);}},[boardSize]);

const playDropSound=useCallback((isSuper=false)=>{if(!soundEnabled||Platform.OS!=='web')return;try{if(!audioContextRef.current)audioContextRef.current=new(window.AudioContext||window.webkitAudioContext)();const ctx=audioContextRef.current,osc=ctx.createOscillator(),gain=ctx.createGain();osc.connect(gain);gain.connect(ctx.destination);osc.frequency.value=isSuper?1200:800;osc.type='sine';gain.gain.setValueAtTime(isSuper?0.25:0.15,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(0.01,ctx.currentTime+(isSuper?0.15:0.08));osc.start(ctx.currentTime);osc.stop(ctx.currentTime+(isSuper?0.15:0.08))}catch(e){}},[soundEnabled]);
